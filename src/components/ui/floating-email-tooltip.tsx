...
interface FloatingEmailTooltipProps {
  isOpen?: boolean;
  onClose?: () => void;
  notifications?: Array<{
    id: string;
...
export function FloatingEmailTooltip({ 
  isOpen: propIsOpen, 
  onClose: propOnClose, 
  notifications = [] 
}: FloatingEmailTooltipProps = {}) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user?.uid) {
      const savedState = localStorage.getItem(`floating-email-tooltip-expanded-${user.uid}`);
      if (savedState !== null) {
  ...
  useEffect(() => {
    if (user?.uid) {
      localStorage.setItem(`floating-email-tooltip-expanded-${user.uid}`, JSON.stringify(isExpanded));
    }
  }, [isExpanded, user?.uid]);

  // Auto-show tooltip on first visit (demo purposes)
  useEffect(() => {
    if (propIsOpen === undefined && user?.uid) {
      const hasSeenTooltip = localStorage.getItem(`floating-email-tooltip-seen-${user.uid}`);
      if (!hasSeenTooltip) {
        setInternalIsOpen(true);
        localStorage.setItem(`floating-email-tooltip-seen-${user.uid}`, 'true');
      }
    }
  }, [user?.uid, propIsOpen]);
      ...
      {propIsOpen === undefined && (
        ...
        >
          <Button
            ...
            className="relative h-12 w-12 rounded-full bg-primary hover:bg-primary/90 shadow-lg hover:shadow-xl transition-all duration-200"
            size="sm"
          >
                    ...
                    <div className="p-4 border-b border-border bg-surface-75/50">
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <ul className="text-xs text-foreground-light space-y-1 ml-4">
